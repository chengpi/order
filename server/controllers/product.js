...
let getfoodList =async(ctx,next)=>{
    let menu = await (sequelize.query("select * from menu",{
    ...
    for(let i=0;i<menu.length;i++){
        let item ={}
        item.name=menu[i].type
        item.foods = await (sequelize.query("select * from foods where type='"+menu[i].id+"' and status =1",{
            type: sequelize.QueryTypes.SELECT
        }));
